...
class SQLDatasetSimilarity:
    ...
    def compute_user_and_item_visits(self, select_from, user_col, item_col):
        ...
        visit_count.select(
            Column("*").count().over(Window(partition_by=[Column(user_col)])),
            alias="nb_visit_user",
        )
        visit_count.select(
            Column("*").count().over(Window(partition_by=[Column(item_col)])),
            alias="nb_visit_item",
        )
        return visit_count
    ...
    def compute_normalise_total_visits(self, select_from, user_col, item_col):
        ...
        normed_count.select(
            ratings.div(
                ratings.times(ratings)
                .sum()
                .over(Window(partition_by=[Column(user_col, table_name="visit_count")]))
                .sqrt()
            ),
        ...
        normed_count.select(
            ratings.div(
                ratings.times(ratings)
                .sum()
                .over(Window(partition_by=[Column(item_col, table_name="visit_count")]))
                .sqrt()
            ),
    ...
    def compute_cf_scores(self, select_from, user_col, item_col):
        ...
        item_cf.select(
            Column("similarity_item", table_name="top_items").sum(),
            alias="item_based_cf_score",
        )
